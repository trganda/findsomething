...
public class Dashboard extends JSplitPane {
    ...
    private void setupTable() {
        informationPane.getInfoTable().addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                int row = informationPane.getInfoTable().rowAtPoint(e.getPoint());
                if (row >= 0) {
                    String info = (String) informationPane.getInfoTable().getModel().getValueAt(row, 1);
                    ...
                    SwingWorker<List<Object[]>, Void> worker = new SwingWorker<>() {
                        ...
                        @Override
                        protected void done() {
                            requestSplitFrame.getRequestDetailsTableModel().getDataVector().removeAllElements();
        ...
        requestSplitFrame.getRequestDetailsTable().addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                int row = requestSplitFrame.getRequestDetailsTable().rowAtPoint(e.getPoint());
                if (row >= 0) {
                    String path = (String) requestSplitFrame.getRequestDetailsTable().getValueAt(row, 1);
                    String host = (String) requestSplitFrame.getRequestDetailsTable().getValueAt(row, 2);
